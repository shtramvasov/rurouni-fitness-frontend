...
export function TrainingProgramCard({ program, verbose, open = true, handleClick }) {
  ...
  useEffect(() => {
    const sortedExercises = Array.from(program.exercises).sort((a, b) => a.exercise_order - b.exercise_order);

    
    const groupedByRoom = sortedExercises.reduce((groups, exercise) => {
      const room = exercise.room || 1;
      if (!groups[room]) groups[room] = [];
      
      groups[room].push(exercise);
      return groups;
    }, {});

    // Преобразуем в массив объектов комнат
    const roomArray = Object.entries(groupedByRoom).map(([roomNumber, exercises]) => ({
    ...
    >
      ...
      <Collapse in={isOpen} sx={{ width: '100%',  }}>
        {roomGroups.map((room, roomIndex) => (
          <Grid2 key={roomIndex} container 
            sx={{ 
              flexGrow: 1, 
              border: `solid 1px`,
              borderRadius: 1,
              borderColor: roomGroups.length > 1 ? theme.palette.gray[200] : 'transparent',
              mb: roomGroups.length > 1 && roomIndex < roomGroups.length - 1 ? 2 : 0
            }}
          >
